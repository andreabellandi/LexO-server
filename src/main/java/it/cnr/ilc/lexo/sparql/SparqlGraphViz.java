...
public class SparqlGraphViz {
    ...
    public static final String GRAPH_VIZ_SENSE_LINKS
            ...
            + "(GROUP_CONCAT(concat(str(?inhypernym),\":\",?inhypernymWr);SEPARATOR=\";\") AS ?in" + SparqlVariable.HYPERNYM + "Grouped)\n"
            + "(GROUP_CONCAT(concat(str(?outhypernym),\":\",?outhypernymWr);SEPARATOR=\";\") AS ?out" + SparqlVariable.HYPERNYM + "Grouped)\n"
            + "(GROUP_CONCAT(concat(str(?inhyponym),\":\",?inhyponymWr);SEPARATOR=\";\")  AS ?in" + SparqlVariable.HYPONYM + "Grouped)\n"
            + "(GROUP_CONCAT(concat(str(?outhyponym),\":\",?outhyponymWr);SEPARATOR=\";\") AS ?out" + SparqlVariable.HYPONYM + "Grouped)\n"
            + "(GROUP_CONCAT(concat(str(?inpartHolonym),\":\",?inpartHolonymWr);SEPARATOR=\";\") AS ?in" + SparqlVariable.HOLONYM + "Grouped)\n"
            + "(GROUP_CONCAT(concat(str(?outpartHolonym),\":\",?outpartHolonymWr);SEPARATOR=\";\") AS ?out" + SparqlVariable.HOLONYM + "Grouped)\n"
            + "(GROUP_CONCAT(concat(str(?inpartMeronym),\":\",?inpartMeronymWr);SEPARATOR=\";\") AS ?in" + SparqlVariable.MERONYM + "Grouped)\n"
            + "(GROUP_CONCAT(concat(str(?outpartMeronym),\":\",?outpartMeronymWr);SEPARATOR=\";\") AS ?out" + SparqlVariable.MERONYM + "Grouped)\n"
            + "(GROUP_CONCAT(concat(str(?outsynonym),\":\",?outsynonymWr);SEPARATOR=\";\") AS ?out" + SparqlVariable.SYNONYM + "Grouped)\n"
            + "(GROUP_CONCAT(concat(str(?insynonym),\":\",?insynonymWr);SEPARATOR=\";\") AS ?in" + SparqlVariable.SYNONYM + "Grouped)\n"
    ...
    public static final String GRAPH_VIZ_EDGE_GRAPH
            ...
            + "        VALUES ?source { _SOURCE_ } .\n"
            + "        VALUES ?target { _TARGET_ } .\n"
    ...
    public static final String GRAPH_VIZ_MIN_PATH = "PREFIX path: <http://www.ontotext.com/path#>\n"
            ...
            + "PREFIX lexinfo: <http://www.lexinfo.net/ontology/3.0/lexinfo#>\n"
            + "\n"
            + "SELECT ?" + SparqlVariable.PATH_INDEX + " ?" + SparqlVariable.EDGE_INDEX + " ?source ?label ?def ?pos ?relation ?target ?labelTarget ?defTarget ?posTarget ?graph\n"
            + "FROM onto:explicit\n"
            + "WHERE {\n"
            + "    VALUES (?src ?dst) {\n"
    ...
    public static final String GRAPH_VIZ_MIN_PATH_WITH_CONCEPTS = "PREFIX path: <http://www.ontotext.com/path#>\n"
            + "PREFIX vartrans: <http://www.w3.org/ns/lemon/vartrans#>\n"
            + "PREFIX onto: <http://www.ontotext.com/>\n"
            + "PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>\n"
            + "PREFIX skos: <http://www.w3.org/2004/02/skos/core#>\n"
            ...
            + "PREFIX lexinfo: <http://www.lexinfo.net/ontology/3.0/lexinfo#>\n"
            + "\n"
            + "SELECT ?" + SparqlVariable.PATH_INDEX + " ?" + SparqlVariable.EDGE_INDEX + " ?source ?label ?conceptLabel ?def ?pos "
            + "?relation ?target ?labelTarget ?conceptLabelTarget ?defTarget ?posTarget ?graph\n"
            + "FROM NAMED onto:explicit\n"
             + "FROM NAMED onto:implicit\n"
            + "WHERE {\n"
            + "    VALUES (?src ?dst) {\n"
            ...
            + "            lexinfo:partOfSpeech ?pos . }\n"
            + "      SERVICE <_REPO_> {\n"
            + "         OPTIONAL { ?source rdfs:label ?conceptLabel }\n"
            + "      }\n"
            + "      BIND (rdf:predicate(?edge) as ?relation)\n"
            + "      BIND (rdf:object(?edge) as ?target) \n"
            + "        OPTIONAL { ?target skos:definition ?defTarget .\n"
            ...
            + "            lexinfo:partOfSpeech ?posTarget . }\n"
            + "      SERVICE <_REPO_> {\n"
            + "         OPTIONAL { ?target rdfs:label ?conceptLabelTarget }\n"
            + "      }\n"
            + "      OPTIONAL { GRAPH ?graph { ?source ?relation ?target } }\n"
            + "    }\n"
            + "}";
